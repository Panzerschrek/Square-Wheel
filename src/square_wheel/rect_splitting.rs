...
fn split_rect_r(r : &Rect, num_parts: u32, out_parts : &mut Vec<Rect>)
{
	...
	if width >= height
	{
		...
		r0 = Rect{ min: Vec2f::new(r.min.x, r.min.y), max: Vec2f::new(middle, r.max.y) };
		r1 = Rect{ min: Vec2f::new(middle, r.min.y), max: Vec2f::new(r.max.x, r.max.y) };
	}
	else
	{
		let middle = r.max.y * ratio0 + r.min.y * ratio1;
		r0 = Rect{ min: Vec2f::new(r.min.x, r.min.y), max: Vec2f::new(r.max.x, middle) };
		r1 = Rect{ min: Vec2f::new(r.min.x, middle), max: Vec2f::new(r.max.x, r.max.y) };
	}
