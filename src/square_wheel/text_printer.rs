...
)
{
	...
	for c in text.chars()
	{
		...
		if c_num < FIRST_GLYPH || c_num > LAST_GLYPH
		{
			c_num = '?' as usize;
		}

		let glyph_index = c_num - FIRST_GLYPH;
...
{
	// TODO - optimize this. Add unrolling for small scales, add branch with glyph totally inside screen.
	...
	for c in text.chars()
	{
		...
		{
			cur_x = x;
			cur_y += (GLYPH_HEIGHT as i32) * (scale as i32);
			continue;
		}
		...
		for glyph_y in 0 .. GLYPH_HEIGHT
		{
			...
			for glyph_x in 0 .. GLYPH_WIDTH as i32
			{
				if (glyph_line_byte & (1 << ((glyph_x & 7) ^ 7))) != 0
				{
					for pix_y in 0 .. scale
					{
						let dst_y = cur_y + (pix_y as i32) + (glyph_y as i32) * (scale as i32);
						if dst_y < 0 || dst_y >= surface_info.height as i32
						{
							continue;
						}
						for pix_x in 0 .. scale
						{
							let dst_x = cur_x + (pix_x as i32) + (glyph_x as i32) * (scale as i32);
							if dst_x >= 0 && dst_x < (surface_info.width as i32)
							{
								pixels[(dst_x as usize) + (dst_y as usize) * surface_info.pitch] = color;
							}
						}
					}
				}
		...
		cur_x += (GLYPH_WIDTH as i32) * (scale as i32);
	}
}
...
const LAST_GLYPH: usize = FIRST_GLYPH + NUM_GLYPHS - 1;
// 1 bit font bitmap.
